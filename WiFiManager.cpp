...
void WiFiManager::WiFiEvent(WiFiEvent_t event, WiFiEventInfo_t info){
    ...
    if(event == ARDUINO_EVENT_WIFI_STA_DISCONNECTED){
      ...
      #ifdef esp32autoreconnect
      #ifdef WM_DEBUG_LEVEL
        DEBUG_WM(DEBUG_VERBOSE,F("[Event] ARDUINO_EVENT_WIFI_STA_DISCONNECTED, reconnecting"));
        #endif
        WiFi.reconnect();
      #endif
  }
...
void WiFiManager::WiFi_autoReconnect(){
    ...
    // if(_wifiAutoReconnect){
      ...
      WiFiEventFuncCb func = std::bind(&WiFiManager::WiFiEvent,this,_1,_2);
      wm_event_id = WiFi.onEvent(func);
      // RvD: seems missing even though OnEvent is no longer handled!
      WiFi.setAutoReconnect(_wifiAutoReconnect);
    // }
  #endif
}
